...
y1x1 <- '100%'
#library(glm)
model <- glm(y ~ age + job + marital + education + default + balance + housing + loan + contact + duration + campaign + pdays + previous + poutcome, data = train_data,family = binomial())
```
...
Splitting the available data into **training (`r nrow(raw_data) * 0.8` at 80%)** and **testing (`r nrow(raw_data) * 0.2` at 20%)** data sets.
...
confusion <- table(test_data$y, predict(model,test_data[,features], type='response')> 0.5)
